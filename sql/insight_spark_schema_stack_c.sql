...
-- 全栈 C：看板、批注协同、工作台公告、系统配置、性能治理处置表
-- 说明：
--   0) Spring Boot 启动时 com.insightspark.c.service.StackCSchemaInitializer 会执行等价 DDL（与下文一致）。
--   1) 请勿修改全栈 A/B 在 insight_spark_schema_from_repo.sql 中的既有表定义。
--   2) 本文件仅新增下列表；手动执行前请确保已存在库 insight_spark 且 is_user 等 A 侧表已创建。
--   3) 外键指向 is_user.user_id、is_dashboard.id；若环境不便使用外键，可删除各表末尾 CONSTRAINT 段。
--   4) is_perf_intervention 依赖 A 侧 is_sql_audit_log 已存在；性能页「标记处置」会写入该表。
...
-- --------------------------------------------------------
-- C.2 我的看板 / 公共看板（画布 JSON，后续可扩展分享字段）
-- --------------------------------------------------------
...
-- =========================================================
-- 全栈 C · 性能治理中心（管理员端，慢查询处置记录）
-- =========================================================

-- --------------------------------------------------------
-- C.6 性能治理 - 慢查询业务处置（与 is_sql_audit_log.id 逻辑关联）
-- --------------------------------------------------------
